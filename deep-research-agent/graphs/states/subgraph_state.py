...
class ChatMessage(BaseModel):
    ...
    tool_calls: Optional[List[Dict]] = Field(
        default=None, description="Tool calls initiated by assistant"
    )
    tool_results: Optional[List[Dict]] = Field(
        default=None, description="Results from tool execution"
    )
    ...
    metadata: Optional[Dict] = Field(
        default=None,
        description="Additional metadata like tokens, latency, step_duration",
    )
...
class ResearchReviewData(TypedDict):
    ...
    depth and max_reviews are injected per-invocation by
    _parallel_research_node based on the active analysis_level,
    allowing each mode (low/mid/high) to control research breadth
    and review cycles independently.
    """
    ...
    # Analysis depth controls — set from analysis_level at graph entry
    depth: int
    max_reviews: int
...
class AgentGraphState(TypedDict):
    ...
    # --- Deep Analysis Intensity (only relevant for deepsearch mode) ---
    analysis_level: Literal["low", "mid", "high"]
    # --- Progress Tracking ---
    total_agents: int
    ...
    # --- User-Provided API Keys (frontend override, fallback to env defaults) ---
    api_keys: Dict[str, str]
    model_selections: Dict[str, str]

    # --- Edit Mode ---
    edit_instructions: Optional[str]
    edit_section_id: Optional[str]
    edit_section_order: Optional[int]
    edit_chapter_heading: Optional[str]
    edit_old_html: Optional[str]
