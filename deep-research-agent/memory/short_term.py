...
class ShortTermMemory:
    ...
    def __init__(self, db_uri: Optional[str] = None):
        ...
        if self._db_uri:
            try:
                ...
                self._is_async = True
                logger.info(
                    "AsyncShallowPostgresSaver instantiated (pending initialization)"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to configure PostgreSQL checkpointer: {e}. Using in-memory storage."
                )
            ...
            try:
                ...
                if self._db_uri:
                    ...
                    async with await psycopg.AsyncConnection.connect(
                        self._db_uri, autocommit=True
                    ) as conn:
                        checkpointer = AsyncShallowPostgresSaver(conn)
                        await checkpointer.setup()

                logger.info(
                    "ShortTermMemory fully initialized with AsyncShallowPostgresSaver"
                )
            except Exception as e:
                logger.error(f"Failed to initialize AsyncShallowPostgresSaver: {e}")
                logger.warning("Falling back to in-memory MemorySaver")
                self._checkpointer = MemorySaver()
                self._is_async = False
                self._pool = None
    ...
    async def get_conversation_history(
        ...
        try:
            ...
            if checkpoint and checkpoint.checkpoint:
                ...
                logger.info(
                    f"[ShortTermMemory] thread={thread_id}, no checkpoint found"
                )
        except Exception as e:
    ...
    def trim_by_token_count(
        ...
        logger.info(
            f"Trimmed {len(messages) - len(trimmed)} messages to fit token limit"
        )
        return trimmed
    ...
    def smart_trim(self, messages: List[Dict], max_tokens: int = 4000) -> List[Dict]:
        """
        ...
        tool_msgs = [
            m for m in messages if m.get("tool_calls") or m.get("tool_results")
        ]
...
Summary:"""
    ...
    async def extract_facts(self, messages: List[Dict], llm: Any = None) -> List[Dict]:
        """
